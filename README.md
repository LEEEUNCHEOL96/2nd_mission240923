...
## 미비사(선택)항 or 막힌 부분
### 막힌부분 404 error 
1. 회원가입 정보를 넣고 회원가입버튼을누르면 404 error 가 발생...
  - 하지만 db에는 정상적으로 데이터가 저장되는현상..

2. 로그인 역시 저장된 데이터로 로그인버튼을 누르면 404 error 가 발생... 
  -하지만 로그인은 정상적으로 처리가 됨...

3. 새로고침 후 로그인 버튼이 정상적으로 로그아웃 버튼으로 변경되어 로그아웃 버튼을 누르면 404 error 발생... 
  - 하지만 새로고침후 정상적으로 로그아웃 처리됨..... 

위 세가지 오류 해결법을 찾느라 시간소모...
